...
# ADR-0179: Risk normalization = within-repo percentile; one-signal-per-channel encoding
...
**Consequences.** The treemap becomes readable and discriminating on any repo size. Percentile coloring is determinism-safe (ranking already is). Tooltip carries both percentile and absolute, labeled — feeds the prove-it drawer (ADR-0174). Trade-off: percentile hides absolute magnitude at a glance (mitigated by the tooltip + the "absolute" label).
