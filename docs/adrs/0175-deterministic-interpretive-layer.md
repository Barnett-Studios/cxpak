...
# ADR-0175: Deterministic interpretive layer (proven analogues of LLM narration)
...
**Consequences.** Narrative parity with an LLM tool, zero inference. Honest labeling: correlational insights (co-change) are marked `~`, keeping the proof-tick contract (ADR-0174) intact. There is **no `EdgeType::CoChange` graph arm** to reuse — co-change lives only on `index.co_changes: Vec<CoChangeEdge>`; the insight computes the set-difference fresh (small, deterministic).
