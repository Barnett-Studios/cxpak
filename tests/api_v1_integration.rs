...
mod api_v1 {
    ...
    #[tokio::test]
    async fn v1_retrieval_search_returns_hits() {
        let (app, _dir) = test_router();
        let resp = app
            .oneshot(
                Request::builder()
                    ...
                    .uri("/v1/retrieval")
                    .header("content-type", "application/json")
                    .body(Body::from(r#"{"op": "search", "query": "main"}"#))
                    .unwrap(),
            )
            .await
            .unwrap();
        ...
        let body = axum::body::to_bytes(resp.into_body(), usize::MAX)
            ...
            .unwrap();
        let val: serde_json::Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(val.get("query").and_then(|v| v.as_str()), Some("main"));
        assert!(val.get("hits").and_then(|v| v.as_array()).is_some());
    }

    #[tokio::test]
    async fn v1_retrieval_missing_op_is_400() {
        let (app, _dir) = test_router();
        let resp = app
            .oneshot(
                Request::builder()
