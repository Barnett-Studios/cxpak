...
HTTP API at `/v1/` with 12 endpoints, exposed via `cxpak serve`. All routes require Bearer token auth when `--token` is set. Endpoints: `health`, `conventions`, `briefing`, `overview`, `trace`, `diff`, `search`, `api-surface`, `dead-code`, `call-graph`, `predict`, `drift`, `security-surface`, `data-flow` (last 9 are stubs returning `{"status":"not_implemented"}`).
