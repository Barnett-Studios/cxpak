...
HTTP API at `/v1/` with 12 endpoints, exposed via `cxpak serve`. `/v1/health` is GET; all others are POST. Bearer token auth (timing-safe) required on all `/v1/` routes when `--token` is set. All endpoints are now wired to real intelligence functions (as of v2.1.0): `health`, `conventions`, `briefing`, `risks`, `architecture`, `call_graph`, `dead_code`, `predict`, `drift`, `security_surface`, `data_flow`, `cross_lang`. Route names use underscores (e.g., `/v1/security_surface`). Request body limit: 2 MB. Graceful shutdown on SIGTERM/Ctrl-C.

### LSP custom methods

14 `cxpak/*` custom JSON-RPC methods, all wired to real intelligence functions (as of v2.1.1): `health`, `conventions`, `blastRadius`, `overview`, `trace`, `diff`, `search`, `apiSurface`, `deadCode`, `callGraph`, `predict`, `drift`, `securitySurface`, `dataFlow`. Methods with required params (`trace`, `search`, `predict`, `dataFlow`, `blastRadius`) return JSON-RPC `-32603 InternalError` when the param is missing (not a `{"note": "..."}` soft failure). `diagnostics_for_file` emits `WARNING` diagnostics tagged `UNNECESSARY` for every dead symbol the dead-code detector flags in the active file.
