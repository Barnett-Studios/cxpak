...
HTTP API at `/v1/` with 12 POST endpoints, exposed via `cxpak serve`. All routes require Bearer token auth when `--token` is set. Live endpoints: `health`, `conventions`, `briefing`. Stub endpoints (return placeholder JSON): `risks`, `architecture`, `call_graph`, `dead_code`, `predict`, `drift`, `security_surface`, `data_flow`, `cross_lang`. Route names use underscores (e.g., `/v1/security_surface`).
