...
/* CX.h(tag, attrs, children) — safe-by-construction DOM builder.
 * Replaces innerHTML+concat patterns: every attribute goes through
 * setAttribute, every text child through textContent, every element
 * child through appendChild.  No interpolation path can produce
 * attacker-controlled markup, regardless of what the caller passes.
 *
 * Children may be: a string (textContent), an Element (appendChild),
 * a number (coerced to string via String(n)), or null/undefined (skipped).
 * Nested arrays are flattened one level so callers can spread groups.
 *
 * Usage:
 *   var el = CX.h('div', {class: 'cxpak-foo', role: 'tab'}, [
 *     CX.h('span', {class: 'cxpak-label'}, [user.name]),
 *     ' - ',
 *     CX.h('span', null, [count]),
 *   ]);
 *
 * Replaces the entire risk surface of the prior CX.esc-everywhere
 * pattern: a developer who forgets to escape can no longer introduce
 * XSS, because there is no escape-needing path through this API.
 */
CX.h = function(tag, attrs, children) {
  ...
  if (attrs) {
    for (var k in attrs) {
      ...
      var v = attrs[k];
      if (v == null) continue;
      // Allow short alias `class` for className.
      var attr = (k === 'class') ? 'class' : k;
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (Array.isArray(c)) {
        for (var j = 0; j < c.length; j++) {
          ...
          if (typeof cj === 'string' || typeof cj === 'number') {
            el.appendChild(document.createTextNode(String(cj)));
          } else {
            el.appendChild(cj);
          }
        }
...
CX.header = function() {
  ...
  // Safe DOM construction via CX.h — every value goes through
  // textContent, every attribute through setAttribute. No interpolation
  // path that could produce markup.
  h.appendChild(CX.h('span', {class: 'cxpak-logo'}, ['cxpak']));
  h.appendChild(CX.h('span', {class: 'cxpak-repo'}, [CX.meta.repo_name || '']));
  h.appendChild(CX.h('span', {class: 'cxpak-type'}, [CX.meta.visual_type_display || '']));
  if (hs != null) {
    h.appendChild(CX.h('span', {class: 'cxpak-health ' + hc}, [hs.toFixed(1) + '/10']));
  }
  h.appendChild(CX.h('span', {class: 'cxpak-timestamp'}, [CX.meta.generated_at || '']));
...
CX.renderGraph = function(root, data, opts) {
  ...
  var ng = root.append('g').attr('class','cxpak-nodes').selectAll('g').data(nodes).join('g')
    ...
    .attr('tabindex', 0)
    .attr('role', 'button')
    .attr('aria-label', function(d) { return d.aria_label || d.label || d.id || ''; });
...
(dash.health.dimensions || []).forEach(function(dim) {
  ...
  row.appendChild(CX.h('span', {class: 'cxpak-dim-label'}, [name.replace(/_/g, ' ')]));
  var bar = CX.h('div', {class: 'cxpak-dim-bar'}, [
    CX.h('div', {
      class: 'cxpak-dim-fill',
      style: 'width:' + (val * 10) + '%;background:' + CX.dimColor(val),
    }, []),
  ]);
  row.appendChild(bar);
  row.appendChild(CX.h('span', {
    class: 'cxpak-dim-val',
    style: 'color:' + CX.dimColor(val),
  }, [val.toFixed(1)]));
...
if (risks.length === 0) {
  ...
  risks.forEach(function(r) {
    ...
    tr.appendChild(CX.h('td', null, [String(r.blast_radius)]));
    tr.appendChild(CX.h('td', {style: 'color:' + testsColor}, [
      CX.h('span', {'aria-label': testsLabel, title: testsLabel}, [
        r.has_tests ? '\u2713' : '\u2014',
      ]),
    ]));
    tb.appendChild(tr);
...
function legendItem(bg, label, extraStyle) {
  var style = 'background:' + bg + (extraStyle ? ';' + extraStyle : '');
  return CX.h('div', {class: 'cxpak-legend-item'}, [
    CX.h('span', {class: 'cxpak-legend-swatch', style: style}, []),
    label,
  ]);
}
leg.appendChild(legendItem('#06d6a0', 'Healthy module (Level 1) / Low risk (Level 2)', null));
leg.appendChild(legendItem('#ffd166', 'Mid range', null));
leg.appendChild(legendItem('#ef476f', 'Unhealthy module / High risk', null));
leg.appendChild(legendItem('rgba(239,71,111,0.25)', 'God file (stroke overlay)', 'border:1px solid #ef476f'));
leg.appendChild(legendItem('#2a2a50', 'Circular dependency', 'border:1px solid #7b68ee;border-style:dashed'));
...
groups.filter(function(d) { return !d.children; })
  ...
  .on('keydown', function(ev, d) {
    if ((ev.key === 'Enter' || ev.key === ' ') && typeof window.CX !== 'undefined' && typeof window.CX.openInspector === 'function') {
      ev.preventDefault();
      var t = d.data.tooltip || {};
      window.CX.openInspector({
        id: d.data.id,
...
/* legend */
var leg = document.createElement('div'); leg.className = 'cxpak-legend';
function riskLegItem(bg, label) {
  return CX.h('div', {class: 'cxpak-legend-item'}, [
    CX.h('span', {class: 'cxpak-legend-swatch', style: 'background:' + bg}, []),
    label,
  ]);
}
...
badge.appendChild(CX.h('span', {class: 'cxpak-flow-symbol'}, [fl.symbol || '']));
badge.appendChild(CX.h('span', {class: 'cxpak-confidence ' + (fl.confidence || '').toLowerCase()}, [fl.confidence || '']));
if (fl.truncated) {
  badge.appendChild(CX.h('span', {class: 'cxpak-flow-truncated'}, ['Truncated']));
}
...
function flowLegItem(bg, label) {
  return CX.h('div', {class: 'cxpak-legend-item'}, [
    CX.h('span', {class: 'cxpak-legend-swatch', style: 'background:' + bg}, []),
    label,
  ]);
}
flowLeg.appendChild(flowLegItem('#4cc9f0', 'Source'));
flowLeg.appendChild(flowLegItem('#ffd166', 'Transform'));
flowLeg.appendChild(flowLegItem('#ef476f', 'Sink'));
flowLeg.appendChild(flowLegItem('#8888aa', 'Passthrough'));
...
if (steps.length === 0) {
  ...
  emptyMsg.appendChild(CX.h('strong', null, ['No timeline snapshots available']));
  emptyMsg.appendChild(document.createElement('br'));
  emptyMsg.appendChild(document.createTextNode(
    'Insufficient git history for timeline. Run cxpak in a repository with more commits, or generate snapshots with '
  ));
  emptyMsg.appendChild(CX.h('code', null, ['cxpak visual --visual-type timeline']));
  emptyMsg.appendChild(document.createTextNode(' after committing changes.'));
...
function flashKeyEvent(idx) {
  ...
  flash.appendChild(CX.h('strong', null, [e.kind || '']));
  flash.appendChild(document.createElement('br'));
  flash.appendChild(document.createTextNode(e.message || ''));
...
dh.appendChild(CX.h('span', null, [
  'Changed: ',
  CX.h('b', null, [String(df.changed_files.length)]),
  ' files',
]));
dh.appendChild(CX.h('span', null, [
  'Blast radius: ',
  CX.h('b', null, [String(df.blast_radius_files.length)]),
  ' files',
]));
dh.appendChild(CX.h('span', {class: 'cxpak-impact-badge ' + impSev}, [
  'Impact ' + (df.impact_score * 100).toFixed(0) + '%',
]));
dw.appendChild(dh);
...
if (df.new_risks && df.new_risks.length > 0) {
  ...
  rl.appendChild(CX.h('div', {class: 'cxpak-diff-risks-title'}, ['Affected Files']));
  var thead = CX.h('thead', null, [
    CX.h('tr', null, [
      CX.h('th', null, ['File']),
      CX.h('th', null, ['Risk']),
      CX.h('th', null, ['Blast']),
  ...
  df.new_risks.slice(0, 10).forEach(function(r) {
    ...
    var trEl = CX.h('tr', null, [
      ...
      CX.h('td', {style: 'color:' + riskColor}, [r.risk_score.toFixed(2)]),
      CX.h('td', null, [String(r.blast_radius)]),
    ]);
    tbody.appendChild(trEl);
  });
  rl.appendChild(CX.h('table', {class: 'cxpak-risk-table'}, [thead, tbody]));
  dw.appendChild(rl);
}

/* legend */
var dleg = document.createElement('div'); dleg.className = 'cxpak-legend';
function diffLegItem(bg, border, label) {
  return CX.h('div', {class: 'cxpak-legend-item'}, [
    CX.h('span', {
      class: 'cxpak-legend-swatch',
      style: 'background:' + bg + ';border:' + border,
    }, []),
    label,
  ]);
}
dleg.appendChild(diffLegItem('rgba(255,209,102,0.2)', '2px solid #ffd166', 'Changed file'));
dleg.appendChild(diffLegItem('rgba(255,159,67,0.15)', '1.5px solid #ff9f43', 'Blast radius'));
